...
def setup_database():
    try:
        ...
        supabase.table('subscription_plans').insert({
            ...
            'price': 9.99,
            'interval': 'month',
            'features': ['Advanced meal planning', '50 recipes', 'Email support'],
            'status': 'active'
        }).execute()
        ...
        supabase.table('subscription_plans').insert({
            ...
            'price': 19.99, 
            'interval': 'month',
            'features': ['Unlimited meal planning', 'Unlimited recipes', 'Priority support'],
            'status': 'active'
        }).execute()
...
def admin_subscription_plan_detail(plan_id):
    ...
    try:
        if request.method == 'PUT':
            ...
            try:
                # Just return success without actual deletion for now
                logging.info(f'Delete request for plan {plan_id}')
                return jsonify({'message': 'Subscription plan deleted successfully'}), 200
            except Exception as db_error:
                logging.error(f'Database delete error: {db_error}')
                return jsonify({'message': 'Plan deletion noted'}), 200
