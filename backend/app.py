...
from utils.performance import cache_result, measure_time, QueryOptimizer
...
def get_recipes():
    ...
    try:
        ...
        page, per_page, offset = QueryOptimizer.get_pagination_params(request)
        result = supabase.table('recipes').select('*').eq('user_id', user_id).range(offset, offset + per_page - 1).order('created_at', desc=True).execute()
...
@cache_result(ttl=600, key_prefix="discover_recipes")
@measure_time
