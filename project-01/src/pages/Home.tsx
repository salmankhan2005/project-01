...
export const Home = () => {
  ...
  const handleAddMeal = () => {
    ...
    setMeals(prev => ({
      ...prev,
      [selectedDay]: prev[selectedDay] ? [...prev[selectedDay].filter(m => m.time !== selectedMealTime), newMeal] : [newMeal]
    }));
  ...
  const handleUpdateMeal = () => {
    ...
    setMeals(prev => ({
      ...prev,
      [selectedDay]: prev[selectedDay]?.map(m => m.time === editingMeal.time ? updatedMeal : m) || []
    }));
  ...
  const handleDeleteMeal = (mealTime: string, day: string) => {
    setMeals(prev => ({
      ...prev,
      [day]: prev[day]?.filter(m => m.time !== mealTime) || []
    }));
    toast.success('Meal deleted successfully');
  };

  const handleCraftMeal = () => {
    if (!craftMealName) {
      ...
      <main className="container-responsive py-6 space-y-6">
        ...
        {viewMode === 'list' ? (
          ...
          <div className="space-y-6 max-h-[60vh] overflow-y-auto scrollbar-hide">
            {daysOfWeek.map((day) => (
              <Card key={day} className="bg-muted/30 border-border rounded-3xl p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {mealTimes.map(({ name, icon: Icon }) => {
                    ...
                    const meal = dayMeals.find(m => m.time === name);
                    return (
                      <Card 
                        key={name} 
      ...
      <Dialog open={planMonthOpen} onOpenChange={setPlanMonthOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-4">
            ...
            <Button onClick={handlePlanMonth} className="w-full">
              Start Planning
            </Button>
          </div>
        </DialogContent>
      ...
      <Dialog open={craftMealOpen} onOpenChange={setCraftMealOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <Button onClick={handleCraftMeal} className="w-full">
              Save Recipe
            </Button>
          </div>
        </DialogContent>
